...
def get_list():
    ...
    with Session(engine) as session:
        # the scalars make sure we are not receiving Row objects, but proper ORM Todo objects
        res = session.execute(select(Todo)).scalars()#
        return [x.to_dict() for x in res]  ### 


def web_done(id, answer="No Answer"):
